...
#include "simplest_ffmpeg_player_su.h"
        ...
        if(last_block_num != -1) {
            ...
            for(i=0; i<param_data_packets_per_block; ++i) {
                ...
                if(!reconstruction_failed || data_pkgs[i]->valid) {
					...
					RingBuffer_write(rbuf,data_blocks[i] + sizeof(payload_header_t),ph->data_length);//数据写入环形缓冲
                }
            }
...
int read_buffer(void * opaque,uint8_t *buf, int buf_size){
	...
	while(RingBuffer_empty(rbuf))usleep(100);
